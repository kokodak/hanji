...
| QA-005 | Markdown indentation | Create nested lists, indented code blocks, blockquotes with nested content, and mixed indentation. | Markdown indentation is preserved in source and rendered accurately in preview decorations. | Open |
| QA-006 | Inline code isolation | Type `` `ego_path_conflict_candidate` `` and move the cursor away. | The inline code span renders as code, underscores remain literal, and emphasis is not applied inside the code span. | Open |
