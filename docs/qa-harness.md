...
| QA-005 | Markdown indentation | Create nested lists, indented code blocks, blockquotes with nested content, and mixed indentation. | Markdown indentation is preserved in source and rendered accurately in preview decorations. Tests: `src/renderer/src/editor/keymaps.test.ts`, `src/renderer/src/markdown/livePreview.test.ts`. | Fixed pending manual verification |
