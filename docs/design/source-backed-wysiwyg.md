...
Current inline projection starts with plain text, strong spans, and inline code spans. The GPUI app keeps source markers visible: strong content uses a heavier font weight, and inline code markers plus content share the code background. GPUI 0.2.2 can merge line layout runs when only font or background changes, so styled inline runs currently force an invisible decoration boundary in the app renderer. Escapes, nesting, emphasis, links, and parser-grade CommonMark behavior should be added incrementally with source mapping tests.
